...
def parse_command(raw_text: str | None) -> Command:
    ...
    if m := re.fullmatch(r"賣\s*(\S+)", text):
        return Command(action="sell_check", stock=m.group(1))
    ...
    if re.fullmatch(r"早盤|盤前|盤前導航", text):
        return Command(action="morning")
